<?php
	...
	********************************************************************/
	...
	function system_getStatus($force_count = false, $domain_id = SELECTED_DOMAIN_ID) {
		...
		// LISTING REVIEW
		$sql = "SELECT COUNT(*) AS total FROM Review WHERE (approved = 0 OR (responseapproved = 0 AND response != '')) AND item_type = 'listing'";
		$r = $dbObjSecond->query($sql); $row = mysql_fetch_assoc($r);
		$status["lr_pending"] = (int)$row["total"];

		// PROMOTION REVIEW
		if (PROMOTION_FEATURE == "on" && CUSTOM_PROMOTION_FEATURE == "on") {
			$sql = "SELECT COUNT(*) AS total FROM Review WHERE (approved = 0 OR (responseapproved = 0 AND response != '')) AND item_type = 'promotion'";
			$r = $dbObjSecond->query($sql); $row = mysql_fetch_assoc($r);
		...
		// ARTICLE REVIEW
		if (ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on") {
			$sql = "SELECT COUNT(*) AS total FROM Review WHERE (approved = 0 OR (responseapproved = 0 AND response != '')) AND item_type = 'article'";
			$r = $dbObjSecond->query($sql); $row = mysql_fetch_assoc($r);
		...
		if (!$force_count) {
			// MONEY
			$sql = "SELECT COUNT(*) AS total, SUM(transaction_amount) AS amount from Payment_Log WHERE transaction_datetime >= '".date("Y-m-d", mktime(0, 0, 0, date("m"), date("d")-30, date("Y")))."' AND hidden = 'n'";
			$r = $dbObjSecond->query($sql); $row = mysql_fetch_assoc($r);
	...
	function system_getHeaderLogo($inline = true) {
		...
		if (file_exists(EDIRECTORY_ROOT.IMAGE_HEADER_PATH)) {
            ...
            $headerlogo = DEFAULT_URL."/assets/images/img-logo.png";
        }
		return $headerlogo;
	}
	...
	function system_getNoImageStyle($cssfile = false, $getFile = false) {
        ...
        if ($getFile) {
            if (file_exists(EDIRECTORY_ROOT.NOIMAGE_PATH."/".NOIMAGE_NAME.".".NOIMAGE_IMGEXT)) {
                ...
                return DEFAULT_URL."/assets/images/img-logo.png";
            }
        } else {
            if ($cssfile) {
                ...
                if (file_exists(EDIRECTORY_ROOT.NOIMAGE_PATH."/".NOIMAGE_NAME.".".NOIMAGE_IMGEXT)) {
                    ...
                    $noimagestyle = "background: #FFF url('".DEFAULT_URL."/assets/images/img-logo.png') 45% 50% no-repeat;";
                }
            }
            return $noimagestyle;
        }
		...
		if ($array) {
			if (count($array) > 0) {
				foreach ($array as $name=>$value) {
					...
					if ((($pos !== false) && ($pos == 0)) || $name == "order_by") {
                                            if ($value) {
	...
	function system_registerForeignAccount($authArray, $accountType, $attach_account = false, $email_notification = SYSTEM_NEW_PROFILE) {
		...
		if (!($account->getNumber("id"))) {
            ...
            $sitemgr_msg .="<br /><a href=\"".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/account/$accountViewLink.php?id=".$account->getNumber("id")."\" target=\"_blank\">".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/account/$accountViewLink.php?id=".$account->getNumber("id")."</a><br /><br />";
...
//		$total_year = $total_payment_year + $total_invoice_year;
        ...
        if (!$all) {
            $sql = "SELECT SUM(transaction_amount) AS total FROM Payment_Log WHERE transaction_status in ('Completed', 'Approved', 'Aprovado', 'Accepted', 'Success', 'SIMPLEPAYSUCCESS', 'Y') AND transaction_datetime > '".$one_month_ago."' AND hidden = 'n'";
            $result = $dbObjSecond->query($sql);
            if (mysql_num_rows($result) > 0) {
                $row = mysql_fetch_assoc($result);
            ...
            */
            ...
            $sql = "SELECT SUM(transaction_amount) AS total FROM Payment_Log WHERE transaction_status in ('Completed', 'Approved', 'Aprovado', 'Accepted', 'Success', 'SIMPLEPAYSUCCESS', 'Y') AND transaction_datetime > '".$one_week_ago."' AND hidden = 'n'";
            $result = $dbObjSecond->query($sql);
            if (mysql_num_rows($result) > 0) {
                $row = mysql_fetch_assoc($result);
		...
		$sql = "SELECT SUM(transaction_amount) AS total FROM Payment_Log WHERE transaction_status in ('Completed', 'Approved', 'Aprovado', 'Accepted', 'Success', 'SIMPLEPAYSUCCESS', 'Y') AND hidden = 'n'";
		$result = $dbObjSecond->query($sql);
		if (mysql_num_rows($result) > 0) {
			$row = mysql_fetch_assoc($result);
	...
	function system_advancedSearch_getCategories($type = "listing", $category_id = "") {
        ...
        if (THEME_ADVSEARCH_CATEGRADIO) {
            ...
            if ($type == "listing" && !$auxtype) {
                $moduleURL = LISTING_DEFAULT_URL."/".ALIAS_LISTING_ALLCATEGORIES_URL_DIVISOR."/";
            } elseif ($auxtype == "promotion") {
                $moduleURL = PROMOTION_DEFAULT_URL."/".ALIAS_PROMOTION_ALLCATEGORIES_URL_DIVISOR."/";
            } elseif ($type == "event") {
                $moduleURL = EVENT_DEFAULT_URL."/".ALIAS_EVENT_ALLCATEGORIES_URL_DIVISOR."/";
            } elseif ($type == "classified") {
                $moduleURL = CLASSIFIED_DEFAULT_URL."/".ALIAS_CLASSIFIED_ALLCATEGORIES_URL_DIVISOR."/";
            } elseif ($type == "article") {
                $moduleURL = ARTICLE_DEFAULT_URL."/".ALIAS_ARTICLE_ALLCATEGORIES_URL_DIVISOR."/";
            } elseif ($type == "blog") {
                $moduleURL = BLOG_DEFAULT_URL."/".ALIAS_BLOG_ALLCATEGORIES_URL_DIVISOR."/";
            }
    ...
    function system_getFrontendPath($file, $folder = "frontend", $original = false, $moduleFolder = false){
        ...
        if (($file == "header.php" || $file == "footer.php" || $file == "header_menu.php" || $file == "footer_menu.php") && ($folder == "layout" || $folder == "frontend") && file_exists(HTMLEDITOR_FOLDER."/".EDIR_THEME."/".($preview ? "preview_" : "").$file) && !$original){
    ...
    function system_getFormFields($module, $level = "", $field = "") {
        ...
        if (is_string($module) && is_string($theme) && (is_numeric($level) || $field)) {
            ...
            if ($field) {
                $fieldName = "level";
                $sql = "SELECT $fieldName FROM ".ucfirst($module)."Level_Field WHERE theme = '".string_strtolower($theme)."' AND field = ".db_formatString($field);
            } else {
                $fieldName = "field";
                $sql = "SELECT $fieldName FROM ".ucfirst($module)."Level_Field WHERE theme = '".string_strtolower($theme)."' AND level = ".$level;
            }
    ...
    function system_updateFormFields($array, $table) {
        ...
        foreach ($avFields as $avField) {
            ...
            if ($avField == "time") { //event time includes start_time and end_time, let's split it
                $sql = "DELETE FROM ".$table."Level_Field WHERE theme = ".db_formatString(EDIR_THEME)." AND (field = 'start_time' OR field = 'end_time')";
            } else {
                $sql = "DELETE FROM ".$table."Level_Field WHERE theme = ".db_formatString(EDIR_THEME)." AND field = '$avField'";
            }
            $dbObj->query($sql);
    ...
    function system_getLevelDetail($table){
        ...
        $dbMain = db_getDBObject(DEFAULT_DB, true);
        $dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbMain);
        $sql = "SELECT value FROM $table WHERE featured = 'y' AND active = 'y' AND theme = ".db_formatString(EDIR_THEME)." ORDER BY value";
        $result = $dbObj->query($sql);
        if (mysql_num_rows($result) > 0){
            while ($row = mysql_fetch_assoc($result)){
    ...
    function system_getUserActivities($type, $accId) {
        ...
        if ($type == "favorites") {
            ...
            if ($ids) {
                $hasItens = true;
                $sql = "SELECT id FROM Listing_Summary WHERE id IN (".$ids.") AND status = 'A' ORDER BY level, title";
