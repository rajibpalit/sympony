<?php
	...
	function domain_getDropDown($http_host = false, $request_uri = false, $query_string = false , $domain_id = false) {
        ...
        if ((string_strpos($_SERVER["PHP_SELF"], "login.php") !== false) || (string_strpos($_SERVER["PHP_SELF"], "resetpassword.php")) !== false  || (string_strpos($_SERVER["PHP_SELF"], "forgotpassword.php")) !== false) {
            return false;
        }
        ...
        if (mysql_num_rows($result) > 1) {
            ...
            while ($row = mysql_fetch_assoc($result)) {
                ...
                $arrayDomains[$i]["id"] = $row["id"];
                $arrayDomains[$i]["name"] = $domainName;
                $arrayDomains[$i]["onclick"] = $domainOnClick;
                $arrayDomains[$i]["disabled"] = $domainDisabled;
                $i++;
            }
	...
	function domain_checkDropDown() {
		...
		$openPMaddPromotion = ((string_strpos($_SERVER["PHP_SELF"], "deal.php"))&&(!string_strpos($_SERVER["PHP_SELF"], "prefs/deal.php"))&&(!string_strpos($_SERVER["REQUEST_URI"], "?id")))&&!(isset($_GET["id"])&& $_GET["id"]!="");
