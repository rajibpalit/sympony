...
class ListingCategory extends Handle
{
    ...
    function Save($update_friendlyurl = true)
    {
        ...
        if ($symfonyContainer = SymfonyCore::getContainer()) {
            $symfonyContainer->get("listing.category.synchronization")->addUpsert($this->id);
        }
    }
    ...
    function Delete()
    {
        if ($this->id != 0) {
            ...
            if ($symfonyContainer = SymfonyCore::getContainer()) {
                $symfonyContainer->get("listing.category.synchronization")->addDelete($category_ids);
            }
        }
    }
    ...
    public static function GetInfoToApp($array_get, &$aux_returnArray, &$aux_fields, &$items, &$auxTable, &$aux_Where)
    {
        ...
        $aux_fields["active_items"] = "active_listing";
