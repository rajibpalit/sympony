<?
	...
	class ClassifiedLevel {
		...
		function ClassifiedLevel($listAll = false, $domain_id = false) {
            ...
            if (is_array($classifiedLevelAux)) {
                foreach ($classifiedLevelAux as $classifiedLevel) {
                    ...
                    $this->content[]  = $classifiedLevel["content"];
        ...
        function updateValues($name, $active, $detail = "", $images = "", $levelValue, $type = "names", $popular = ""){
            ...
            if ($type == "names") {
                $sql = "UPDATE ClassifiedLevel SET name = '".$name."', active = '".$active."', popular = '".$popular."' WHERE value = ".$levelValue." AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            } elseif ($type == "fields") {
                $sql = "UPDATE ClassifiedLevel SET detail = '".$detail."', images = '".$images."' WHERE value = ".$levelValue." AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            }
        ...
        function updateFeatured($newValue, $level){
            ...
            $sql = "UPDATE ClassifiedLevel SET featured = '{$newValue}' WHERE value = {$level} AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            $dbObj->query($sql);
        }
