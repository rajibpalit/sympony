...
class ArticleCategory extends Handle
{
    ...
    function Save()
    {
        ...
        if ($symfonyContainer = SymfonyCore::getContainer()) {
            $symfonyContainer->get("article.category.synchronization")->addUpsert($this->id);
        }
    }
    ...
    function Delete()
    {
        if ($this->id != 0) {
            ...
            if ($symfonyContainer = SymfonyCore::getContainer()) {
                $symfonyContainer->get("article.category.synchronization")->addDelete($categoryDump);
            }
    ...
    public static function GetInfoToApp($array_get, &$aux_returnArray, &$aux_fields, &$items, &$auxTable, &$aux_Where)
    {
        ...
        $aux_fields["active_items"] = "active_article";
