...
class Location5 extends Handle
{
    ...
    function updateFullTextItemsLoc($listings_ids = false)
    {
        ...
        if (!$listings_ids) {
            ...
            if ($this->id) {
                ...
                $dbObj = db_getDBObject(DEFAULT_DB, true);
                $sql = "SELECT id FROM Listing WHERE Location_5 = $this->id";
                $result = $dbObj->query($sql);

                while ($row = mysql_fetch_array($result)) {
                    if ($row['id']) {
                        $listingObj = new Listing($row['id']);
                        $listingObj->setFullTextSearch(true, 5);
                        unset($listingObj);
                    }
                }
    ...
    function retrieveFeatureds($_locations, $default = false, $last_default_level = false, $last_default_id = false)
    {
        ...
        foreach ($_locations as $each_location) {
            ...
            if ($each_location == 5) {
                $searchReturn["select"][] = "Location_5.* ";
                $searchReturn["orderby"][] = "Location_5.name ";
            } else {
                if ($each_location < 5) {
                    ...
                    $searchReturn["on"][] = "Location_5.location_" . $each_location . "=Location_" . $each_location . ".id ";
                    if ($_locations == $_locations[0]) {
                        $searchReturn["orderby"][] = "Location_5.location_" . $each_location . " ";
                    }
                }
            }
        }
        ...
        $sql .= " WHERE Location_5." . ($default == false ? "id IN (" . $locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID,
                    5) . ")" : "id = $default");

        if ($last_default_id) {
            $sql .= " AND Location_5.location_" . $last_default_level . " = " . $last_default_id . " ";
        }

        $sql .= " GROUP BY Location_5.id ";
        $sql .= " ORDER BY ";
    ...
    {
        $sql = "UPDATE Location_5 SET Location_5.default='n' WHERE Location_5.default='y'";
        $db = db_getDBObject(DEFAULT_DB, true);
        $db->query($sql);
        $this->Save();
    }
    ...
    function getFullFriendlyURL()
    {
        ...
        for ($i = 0; $i < count($locationsLevel); $i++) {
            if (($locationsLevel[$i] != 1) && ($locationsLevel[$i] != 5)) {
                $sqlAux .= "IF (location_" . $locationsLevel[$i] . " > 0, concat((SELECT friendly_url FROM Location_" . $locationsLevel[$i] . " WHERE id = Location_5.location_" . $locationsLevel[$i] . "), '/'), ''),";
            }

        }
        $sql = "SELECT concat((SELECT friendly_url FROM Location_1 WHERE id = Location_5.location_1),'/'," . $sqlAux . "
                                    friendly_url) AS full_friendly_url
                                 FROM Location_5 WHERE id = " . $this->id;
