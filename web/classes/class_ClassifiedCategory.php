...
class ClassifiedCategory extends Handle
{
    ...
    function Save()
    {
        ...
        if ($symfonyContainer = SymfonyCore::getContainer()) {
            $symfonyContainer->get("classified.category.synchronization")->addUpsert($this->id);
        }
    }
    ...
    function Delete()
    {
        if ($this->id != 0) {
            ...
            */
            ...
            if ($symfonyContainer = SymfonyCore::getContainer()) {
                $symfonyContainer->get("classified.category.synchronization")->addDelete($categoryDump);
            }
        }
    ...
    function getFullPath()
    {
        ...
        $fields = "`id`, `category_id`, `active_classified`, `featured`, `enabled`, `friendly_url`, `title`";
    ...
    function countSubCatToApp()
    {
        ...
        if ($this->category_id == 0) {
            $sql = "SELECT id FROM ClassifiedCategory WHERE category_id = " . $this->id;
            $aux_id = $this->id;
        } elseif ($this->id) {
            $sql = "SELECT id FROM ClassifiedCategory WHERE category_id = " . $this->category_id;
        ...
        if ($aux_id) {
            ...
            $update_sql = "UPDATE ClassifiedCategory SET count_sub = " . $total . " WHERE id = " . $aux_id;
    ...
    public static function GetInfoToApp($array_get, &$aux_returnArray, &$aux_fields, &$items, &$auxTable, &$aux_Where)
    {
        ...
        $auxTable = "ClassifiedCategory";

        /**
         * Fields to Classified
         */
        // Label = value (field on DB);
        $aux_fields["id"] = "id";
        $aux_fields["title"] = "title";
        $aux_fields["active_items"] = "active_classified";
