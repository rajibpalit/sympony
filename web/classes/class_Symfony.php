...
class Symfony
{
    ...
    private function saveMultiDomain(array $value)
    {
        ...
        if ($newDomain === true || $rebuildElastic === true) {
            ...
            $this->createElasticIndex($_conf['elastic'], $_conf['database'], $_conf['locale']);
        }

        $this->fileValue['multi_domain'] = $config;
    }

    private function createElasticIndex($index, $database, $language = "en_us")
    {
        ...
        /* Deletes rivers and index to the domain */
        $this->removeElastic($index, $riverFiles);

        // Sends request to create a new index
        $indexFile = file_get_contents($jsonBasePath.'/IndexCreation.json');
        $language = SymfonyCore::getContainer()->get("search.engine")->getAnalyzerForLanguage($language);
        $indexFile = str_replace("%language%", $language, $indexFile);
        $this->elasticExec('post', $url, 'Error on create a new index', ['body' => $indexFile]);

        // Sends request to load the locations in index
        SymfonyCore::rebuildElasticsearchLocations();

        /* Creates rivers */
        $riverConfig = [
            'mysql_host'     => SymfonyCore::getContainer()->getParameter('database_host'),
            'mysql_user'     => SymfonyCore::getContainer()->getParameter('database_user'),
            'mysql_pass'     => SymfonyCore::getContainer()->getParameter('database_password'),
        ...
        foreach ($riverFiles as $river => $file) {
            ...
            // Change keys
            foreach ($riverConfig as $key => $value) {
                $body = str_replace("%{$key}%", $value, $body);
            }

            $this->elasticExec('post', $riverUrl.$river.'/_meta', 'Error on create rivers',
