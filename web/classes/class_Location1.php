...
class Location1 extends Handle
{
    ...
    function Save($updFullText = false)
    {
        ...
        if ($symfonyContainer = SymfonyCore::getContainer()){
             $symfonyContainer->get("location.synchronization")->addLocation1Upsert($this->id);
        }
    }
    ...
    function Delete($updFullText = false, $updateOnElasticsearch = true)
    {
        if ($this->id) {
            ...
            if ($updateOnElasticsearch and $symfonyContainer = SymfonyCore::getContainer()){
             $symfonyContainer->get("location.synchronization")->addLocation1Delete($this->id);
         }
        }
    }
    ...
    function retrieveFeatureds(
        ...
        $sql = "
				...
				FROM
					Location_1
				WHERE
					Location_1." . ($default == false ? "id IN (" . $locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID,
                    1) . ")" : "id = $default") . "
				GROUP BY
					Location_1.id
				ORDER BY
    ...
    {
        $dbObj = db_getDBObject(DEFAULT_DB, true);
        $sql = "SELECT $fields FROM Location_1 " . (is_array($ids) ? "WHERE id IN (" . implode(",",
                    $ids) . ")" : "") . " ORDER BY name";
        $result = $dbObj->query($sql);
        while ($row = mysql_fetch_assoc($result)) {
            $data[] = $row;
    ...
    {
        $dbObj = db_getDBObject(DEFAULT_DB, true);
        $sql = "SELECT friendly_url AS full_friendly_url FROM Location_1 WHERE id = " . $this->id;
        $result = $dbObj->query($sql);
