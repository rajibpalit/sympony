<?
	...
	class ListingLevel {
        ...
        function updateValues($name = "", $active = "", $has_promotion = "", $has_review = "", $has_sms = "", $has_call = "", $backlink = "", $detail = "", $images = "", $levelValue, $type = "names", $popular = ""){
            
            $dbMain = db_getDBObject(DEFAULT_DB, true);
            $dbObj = db_getDBObjectByDomainID(SELECTED_DOMAIN_ID, $dbMain);
            
            if ($type == "names") {
                $sql = "UPDATE ListingLevel SET name = '".$name."', active = '".$active."', popular = '".$popular."' WHERE value = ".$levelValue." AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            } elseif ($type == "fields") {
                $sql = "UPDATE ListingLevel SET detail = '$detail', has_promotion = '".$has_promotion."', has_review = '".$has_review."', has_sms = '".$has_sms."', has_call = '".$has_call."', backlink = '".$backlink."', images = '".$images."' WHERE value = ".$levelValue." AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            }
        ...
        function updateFeatured($newValue, $level){
            ...
            $sql = "UPDATE ListingLevel SET featured = '{$newValue}' WHERE value = {$level} AND theme = '".(EDIR_THEME ? EDIR_THEME : "default")."'";
            $dbObj->query($sql);
        }
