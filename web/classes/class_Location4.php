...
class Location4 extends Handle
{
    ...
    function Save($updFullText = false)
    {
        ...
        if ($symfonyContainer = SymfonyCore::getContainer()) {
            $symfonyContainer->get("location.synchronization")->addLocation4Upsert($this->id);
        }
    }
    ...
    function Delete($updFullText = false, $updateOnElasticsearch = true)
    {
        if ($this->id) {
            ...
            if ($updateOnElasticsearch and $symfonyContainer = SymfonyCore::getContainer()) {
                $symfonyContainer->get("location.synchronization")->addLocation4Delete($this->id);
            }
        }
    }
    ...
    function retrieveFeatureds($_locations, $default = false, $last_default_level = false, $last_default_id = false)
    {
        ...
        $sql .= " WHERE Location_4." . ($default == false ? "id IN (" . $locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID,
                    4) . ")" : "id = $default");
    ...
    {
        $dbObj = db_getDBObject(DEFAULT_DB, true);
        $sql = "SELECT $fields FROM Location_4 " . (is_array($ids) ? "WHERE id IN (" . implode(",",
                    $ids) . ")" : "") . " ORDER BY name";
        $result = $dbObj->query($sql);
        while ($row = mysql_fetch_assoc($result)) {
            $data[] = $row;
    ...
    function retrieveLocationByLocation($father_level, $allLocations = false, $ids = false)
    {
        ...
        if ($allLocations) {
            ...
            if (is_array($locations_id)) {
                $sql = "SELECT * FROM Location_4 WHERE location_" . $father_level . " = " . $father_level_value . " AND id IN (" . implode(",",
                        $locations_id) . ") ORDER BY NAME";
            } else {
                return false;
            }
        } else {
            $sql = "SELECT * FROM Location_4 WHERE location_" . $father_level . " = " . $father_level_value . " " . (is_array($ids) ? " AND id IN (" . implode(",",
                        $ids) . ")" : "") . " ORDER BY NAME";
        }
    ...
    function getFullFriendlyURL()
    {
        ...
        for ($i = 0; $i < count($locationsLevel); $i++) {
            if (($locationsLevel[$i] > 1) && ($locationsLevel[$i] < 4)) {
                $sqlAux .= "IF (location_" . $locationsLevel[$i] . " > 0, concat((SELECT friendly_url FROM Location_" . $locationsLevel[$i] . " WHERE id = Location_4.location_" . $locationsLevel[$i] . "), '/'), ''),";
            }
        }
        $sql = "SELECT concat((SELECT friendly_url FROM Location_1 WHERE id = Location_4.location_1),'/'," . $sqlAux . "
                                    friendly_url) AS full_friendly_url
                                 FROM Location_4 WHERE id = " . $this->id;
