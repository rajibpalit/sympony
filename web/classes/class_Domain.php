<?
	...
	class Domain extends Handle {
			...
			if (mysql_num_rows($result) > 0) {
				...
				if ($sample_link) {
					...
					if ($new_link) {
						...
						$dbObj->Query($sqlCC);
                        $sqlCC = "INSERT INTO `Control_Cron` (`domain_id`, `running`, `last_run_date`, `type`) VALUES ($this->id, 'N', NOW(), 'count_locations');";
						$dbObj->Query($sqlCC);
                        ...
                        // Set the Percentage to 71%
						// Copy FAQ data to Domain Database
						if (!$this->checkError()) {
							$this->incFilePercentage(system_showText(LANG_SITEMGR_DOMAIN_PROCESS_COPYING_FAQ), "71");
							$this->copyFAQToDomain($new_dbObj);
						} else {
							if (!$this->new_db_closed) {
								$this->new_db_closed = true;
								$new_dbObj->Close();
		...
		function copyLevelToDomain($module, &$new_dbObj) {
			$levelFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/level/".$module."_level.sql";
			if ($levelFile = fopen($levelFilePath, "r")) {
				...
				if (!mysql_error()) {
                    ...
                    if ($module == "classified" || $module == "event" || $module == "listing"){
                        $levelFieldFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/level/".$module."_level_field.sql";
		...
		function copySettingGoogleToDomain (&$new_dbObj) {
			$googleFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/setting/setting_google.sql";
		...
		function copySettingLocationToDomain (&$new_dbObj) {
			$locationFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/setting/setting_location.sql";
        ...
        function copyFAQToDomain (&$new_dbObj) {
			...
			$sql = "SELECT `database_name` FROM `Domain` WHERE `status` = 'A' ORDER BY `id` LIMIT 1";
			$result = $dbObj->query($sql);
			...
			if (mysql_num_rows($result) > 0) {
                ...
                $sqlFAQ = "INSERT INTO `FAQ` SELECT * FROM `$dbName`.`FAQ`;";
                $new_dbObj->Query($sqlFAQ);
		...
		function copyContentToDomain(&$new_dbObj) {
			$contentFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/content/content.sql";
		...
		function copyCustomTextToDomain(&$new_dbObj) {
			...
			$customTLangFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/custom_text/custom_text.sql";
		...
		function copyEmailNotificationToDomain (&$new_dbObj) {
			$emailNFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/email_notification/email_notification.sql";
			if ($emailNFile = fopen($emailNFilePath, "r")) {
				...
				$emailNDFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/email_notification/email_notification_default.sql";
		...
		function copyListingTemplateToDomain (&$new_dbObj) {
			$listingTemplateFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/listingtemplate/listingtemplate.sql";
		...
		function copyLangToDomain(&$new_dbObj) {
			$langFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/lang/lang.sql";
		...
		function copySettingPaymentToDomain(&$new_dbObj) {
			$paymentFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/setting/setting_payment.sql";
		...
		function copySettingSocialNetworkToDomain(&$new_dbObj) {
			$settingFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/setting/setting_social_network.sql";
		...
		function copySettingToDomain(&$new_dbObj) {
			$settingFilePath = EDIRECTORY_ROOT."/".SITEMGR_ALIAS."/sites/default_script/setting/setting.sql";
			if ($settingFile = fopen($settingFilePath, "r")) {
				$settingSQL = fread($settingFile, filesize($settingFilePath));
				fclose($settingFile);
				$new_dbObj->Query($settingSQL);
