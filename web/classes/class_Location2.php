...
class Location2 extends Handle
{
    ...
    function updateFullTextItemsLoc($listings_ids = false)
    {
        if (!$listings_ids) {
            ...
            if ($this->id) {
                ...
                $dbObj = db_getDBObject(DEFAULT_DB, true);
                $sql = "SELECT id FROM Listing WHERE Location_2 = $this->id";
                $result = $dbObj->query($sql);
    ...
    function retrieveFeatureds($_locations, $default = false, $last_default_level = false, $last_default_id = false)
    {
        ...
        $sql .= " WHERE Location_2." . ($default == false ? "id IN (" . $locationFeatObj->getFeatureds(SELECTED_DOMAIN_ID,
                    2) . ")" : "id = $default");

        if ($last_default_id) {
            $sql .= " AND Location_2.location_" . $last_default_level . " = " . $last_default_id . " ";
        }
    ...
    {
        $dbObj = db_getDBObject(DEFAULT_DB, true);
        $sql = "SELECT concat((SELECT friendly_url FROM Location_1 WHERE id = Location_2.location_1),'/',friendly_url) AS full_friendly_url FROM Location_2 WHERE id = " . $this->id;
        $result = $dbObj->query($sql);
