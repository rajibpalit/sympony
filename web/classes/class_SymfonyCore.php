...
class SymfonyCore
{
    ...
    public static function initialize()
    {
        if (!self::$kernel) {
            ...
            $bootstrapCacheFile = EDIRECTORY_ROOT . "{$s}..{$s}app{$s}bootstrap.php.cache";
    ...
    public static function rebuildElasticsearchLocations()
    {
        ...
        /* Asynchronous call to location update controller, after setting the permission flag in database */
        try {
            $domainDatabase = DatabaseHandler::getDomainConnection();
            $statement = $domainDatabase->prepare("INSERT INTO Setting (`name`, `value`) VALUES (:name, :value)");
            $statement->execute([":name" => "elasticsearchRebuildLocations", ":value" => true]);
        } catch (\PDOException $e) {
        }

        $controllerUrl = $domain->url . "/Y2Fyb2xpbmFz/rebuildLocations";

        try {
            $guzzleClient = new \GuzzleHttp\Client();
            $guzzleClient->get($controllerUrl, ['future' => true]);
        } catch (Exception $e) {
            SymfonyCore::getContainer()->get("logger")->critical("Elasticsearch Synchronization Failure", ["exception" => $e]);
        }
    }
