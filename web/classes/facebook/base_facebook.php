...
abstract class BaseFacebook
{
  ...
  protected function _graph($path, $method = 'GET', $params = array()) {
    ...
    if (is_array($result) && isset($result['error'])) {
      //CR 78034 - Error code 200 = The user hasn't authorized the application to post in his behalf. Do not throw any exception in this case.
  ...
  protected function makeRequest($url, $params, $ch=null) {
    ...
    if (ini_get('display_errors') && curl_error($ch)) {
        echo curl_error($ch); 
    }
  ...
  public static function getFBInstance (&$facebook) {
		if (!isset($facebook) || !$facebook instanceof Facebook) {
			$params = array(
  ...
  /**
	 ...
	 * 		"facebook_action"	=> $_GET["facebook_action]
	 *  );
	 * 	$facebook->getUserInfo($user_details, $extraInfo);
	 * 
	 * @author Arca Solutions, Inc
	 * @access Public
	 * @version 9.0
	 ...
	 */
  public function getUserInfo(&$userInfo, $extraInfo = false) {
    if ($this->getUser()) {
      try {
        ...
        $api_r = '/v2.4/me' . '/?fields=' . $_fields;

        $fbMainInfo = $this->api($api_r);

        foreach ($fields as $key => $value)
        {
          $userInfo[$key] = $fbMainInfo[$value];
        }

        $fbMusics = $fbMainInfo["music"];
        $fbMovies = $fbMainInfo["movies"];
        $fbBooks = $fbMainInfo["books"];
        $fbSports = $fbMainInfo["sports"];
        $fbEmail = $fbMainInfo["email"];

        $userInfo["picture"] = "http://graph.facebook.com/" . $fbMainInfo["id"] . "/picture?type=large";

        //$userInfo['fbMainInfo'] = $fbMainInfo;

        if (validate_email($fbEmail)) {
        ...
        if ($fbMusics["data"] && is_array($fbMusics["data"]) && $fbMusics["data"][0]) {
          unset($userInfo["favorite_musics"], $arrMusic);
          foreach ($fbMusics["data"] as $fbMusic) {
            $arrMusic[] = $fbMusic["name"];
          }
          $userInfo["favorite_musics"] = implode(", ", $arrMusic);
        }

        if ($fbMovies["data"] && is_array($fbMovies["data"]) && $fbMovies["data"][0]) {
          unset($userInfo["favorite_movies"], $arrMovie);
          foreach ($fbMovies["data"] as $fbMovie) {
            $arrMovie[] = $fbMovie["name"];
          }
          $userInfo["favorite_movies"] = implode(", ", $arrMovie);
        }

        if ($fbBooks["data"] && is_array($fbBooks["data"]) && $fbBooks["data"][0]) {
          unset($userInfo["favorite_books"], $arrBook);
          foreach ($fbBooks["data"] as $fbBook) {
            $arrBook[] = $fbBook["name"];
          }
          $userInfo["favorite_books"] = implode(", ", $arrBook);
        }

        if ($fbSports && is_array($fbSports) && $fbSports[0]) {
          unset($userInfo["favorite_sports"], $arrSport);
          foreach ($fbSports as $fbSport) {
            $arrSport[] = $fbSport["name"];
          }
          $userInfo["favorite_sports"] = implode(", ", $arrSport);
        }
      } catch (FacebookApiException $e) {
        error_log($e);
      }
    ...
    /**
	 * Generate the metatag for facebook commenting moderation according to type param
	 * Eg:
	 * 	$params = array (
	 * 		"app_id"	=> "123456789"
	 *  );
	 * 	Facebook::getMetaTags("admins", $params);
	 * 
	 * @author Arca Solutions, Inc
	 * @access Public
	 * @version 9.0
	 ...
	 * @return string $returnTag
	 */
	public static function getMetaTags($type, $params) {
		unset($bOpenTag, $bCloseTag);
		switch ($type) {
			case "admins":
				$bOpenTag = "<meta property=\"fb:admins\" content=\"[PARAMS]\"";
				break;
			case "app_id":
				$bOpenTag = "<meta property=\"fb:app_id\" content=\"[PARAMS]\"";
				break;
		}
		$bCloseTag = " />";
		if (is_array($params)) {
