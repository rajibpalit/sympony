...
//						}
		...
		function getFriendlyURL($mobile = false, $module_url = false, $field = "friendly_url", $show_html = "on") {
			...
			return ($module_url ? $module_url."/" : "").$this->$field.($show_html == "on" ? ".html" : "");
		 ...
		 * @param boolean $mobile
		 */
        public static function getPopularCategories($limit, $table = "ListingCategory", $field = "active_listing") {
            ...
            $sql = "SELECT id, title, $field as active_item ".($table == "ListingCategory" ? ", full_friendly_url" : ", friendly_url AS full_friendly_url" )." FROM $table WHERE $field > 0 AND category_id = 0 AND featured = 'y' AND enabled = 'y' ORDER BY $field DESC, title LIMIT ".$limit;
            $result = $db->query($sql);
