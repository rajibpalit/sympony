    ...
    */
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($modules_options && !DEMO_LIVE_MODE) {
            ...
            $modules = array();
            $modules_array = array('article', 'banner', 'blog', 'classified', 'event', 'promotion');
            foreach ($modules_array as $module) {
                if (!is_null(${'check_'.$module.'_feature'}))
                    $modules['modules.available'][] = str_replace('promotion','deal',$module);
            }
            $modules['modules.available'] = count($modules['modules.available']) ? implode(',',$modules['modules.available']) : '';

            $symfony->save('Configs', array('parameters' => $modules));
        ...
        if ($visitor) {
            ...
            $buffer = "<?".PHP_EOL;
            $buffer .= "	define(\"SOCIALNETWORK_FEATURE\", \"".($enable_visitorprofile ? "on" : "off")."\");".PHP_EOL;
            $buffer .= "?>".PHP_EOL;
    ...
    $socialObj = new SettingSocialNetwork('',SELECTED_DOMAIN_ID);
    $modulesVisitor = array("general", "listing", "article");
