    ...
    */
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if ($action == "delete") {
            ...
            $category->delete();
            $message = 1;
            header("Location: ".DEFAULT_URL."/".SITEMGR_ALIAS."/content/".CLASSIFIED_FEATURE_FOLDER."/categories/index.php?message=".$message."&screen=$screen&letter=$letter".(($url_search_params) ? "&$url_search_params" : "")."");
            exit;
        }
    }
	...
	# PAGE BROWSING
	# ----------------------------------------------------------------------------------------------------
	$pageObj  = new pageBrowsing("ClassifiedCategory", $screen, RESULTS_PER_PAGE, "title, id", "title", $letter, "category_id = ".db_formatNumber($category_id), "id, `title`, enabled");
