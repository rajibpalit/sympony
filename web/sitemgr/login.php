	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		if (sess_authenticateSM($_POST["username"], $_POST["password"], $authmessage)) {
			...
			if ($destiny) {
				...
				$url = ((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : DEFAULT_URL)."/".SITEMGR_ALIAS."/";
			}
		...
		if ($forgotPasswordObj->getString("unique_key") && ($forgotPasswordObj->getString("section") == "sitemgr")) {
			...
			if ($sitemgr_username) {
				...
				header("Location: ".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : DEFAULT_URL)."/".SITEMGR_ALIAS."/resetpassword.php?key=".$_GET["key"]);
				exit;
