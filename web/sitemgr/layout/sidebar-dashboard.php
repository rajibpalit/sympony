    ...
    <div class="sidebar togglepush" id="sidebar" role="navigation">
        ...
        <div class="main-sidebar nano">
            <ul class="nav nav-pills nav-stacked nano-content">
                ...
                <li id="navDesign" >
                    <a href="<?=DEFAULT_URL?>/<?=SITEMGR_ALIAS?>/design/layout-editor/"><i class="icon-paint14"></i><?=system_showText(LANG_SITEMGR_DESIGN_CUSTOM);?></a>
                </li>
                <? } ?>
                ...
                <li id="navAppbuilder" >
                    ...
                    <? if ($appPending == "yes") { ?>
                    <span class="notification"><a href="<?=DEFAULT_URL."/".SITEMGR_ALIAS."/mobile/appbuilder/connect.php?download=yes"?>" target="_blank"  title="<?=str_replace("[x]", $downloadsPending, system_showText(($downloadsPending == 1 ? LANG_SITEMGR_BUILDER_APPREADY_SING : LANG_SITEMGR_BUILDER_APPREADY)));?>"><?=$downloadsPending?></a></span>
                    <? } ?>
                </li>
                <? } ?>
                ...
                <li class="branding-message">
                    Powered by <a href="http://www.edirectory.com<?=(string_strpos($_SERVER["HTTP_HOST"], ".com.br") !== false ? ".br" : "")?>" target="_blank">eDirectory Cloud Service</a>&trade; <?=VERSION?>. &copy; Arca Solutions Inc.
                </li>
                <? } ?>
            </ul>
        </div>
