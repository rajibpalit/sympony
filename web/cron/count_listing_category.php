...
$sqlDomain = "	SELECT
					...
					WHERE CC.`running` = 'N'
					AND CC.`type` = 'count_listing_category'
					AND D.`status` = 'A'
					$sql_period
					ORDER BY
...
if (mysql_num_rows($resDomain) > 0) {
    ...
    $sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'Y', `last_run_date` = NOW() WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_listing_category'";
...
$sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'N' WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_listing_category'";
mysql_query($sqlUpdate, $link);

////////////////////////////////////////////////////////////////////////////////////////////////////
$time_end = getmicrotime();
$time = $time_end - $time_start;
print "Count " . string_ucwords(LISTING_FEATURE_NAME) . " Category Update on Domain " . SELECTED_DOMAIN_ID . " - " . date("Y-m-d H:i:s") . " - " . round($time,
        2) . " seconds.\n";
////////////////////////////////////////////////////////////////////////////////////////////////////
