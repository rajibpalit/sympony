                        ...
                        foreach ($allLevels as $eachLevelValue => $eachLevelName) {
                            if ($row["listing_level"]) {
                                if (string_strpos(string_strtoupper($eachLevelName),
                                        string_strtoupper($row["listing_level"])) !== false
                                ) {
...
//                        $listing_friendly_url = $listing_friendly_url . FRIENDLYURL_SEPARATOR . uniqid();
                        ...
                        if ($needToUpdate) {
                            ...
                            $sqlListingSummary .= "description = " . db_formatString($row["listing_description"], "",
                                    true) . ", ";
                            if ($fulltextsearch_keyword) {
                                $sqlListingSummary .= "fulltextsearch_keyword = " . db_formatString(implode(" ",
                                        $fulltextsearch_keyword), "", true) . ", ";
                            }
                            if ($fulltextsearch_where) {
                                $sqlListingSummary .= "fulltextsearch_where = " . db_formatString(implode(" ",
                                        $fulltextsearch_where), "", true) . ", ";
                            }
                            ...
                            */
                            ...
                            for ($i = 0; $i < count($locField); $i++) {
                                ...
                                if ($objAux) {
                                    ...
                                    $listingSummaryObj->setString(trim($locField[$i]) . "_title",
                                        $locObj->getString("name"));
                                    $listingSummaryObj->setString(trim($locField[$i]) . "_abbreviation",
                                        $locObj->getString("abbreviation"));
                                    $listingSummaryObj->setString(trim($locField[$i]) . "_friendly_url",
                                        $locObj->getString("friendly_url"));
                                }
                            }
                            ...
                            if ($fulltextsearch_keyword) {
                                $listingSummaryObj->setString("fulltextsearch_keyword",
                                    implode(" ", $fulltextsearch_keyword));
                            }
                            if ($fulltextsearch_where) {
                                $listingSummaryObj->setString("fulltextsearch_where", implode(" ", $fulltextsearch_where));
                            }
