...
# ----------------------------------------------------------------------------------------------------
# * FILE: /cron/count_classified_category.php
# ----------------------------------------------------------------------------------------------------

##################################################
# THIS SCRIPT IS ONLY NECESSARY TO COUNT CLASSIFIEDS BY CATEGORY
# WHEN CLASSIFIEDS ARE INSERTED/UPDATED OUT OF EDIRECTORY PROCCESS.
# IF IT HAPPENED, RUN THIS SCRIPT ONLY ONE TIME. THE CODE ALREADY
# HANDLES THE NUMBER OF ACTIVE CLASSIFIEDS BY CATEGORY. TO RUN THIS
...
////////////////////////////////////////////////////////////////////////////////////////////////////
if (COUNT_PERIOD != "") {
    $sql_period = " AND (ADDDATE(CC.`last_run_date`, INTERVAL " . COUNT_PERIOD . ") <= NOW() OR CC.`last_run_date` = '0000-00-00 00:00:00') ";
} else {
    $sql_period = "";
}
$sqlDomain = "	SELECT
					...
					WHERE CC.`running` = 'N'
					AND CC.`type` = 'count_classified_category'
					AND D.`status` = 'A'
					$sql_period
					ORDER BY
...
if (mysql_num_rows($resDomain) > 0) {
    ...
    $sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'Y', `last_run_date` = NOW() WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_classified_category'";
...
////////////////////////////////////////////////////////////////////////////////////////////////////
system_countActiveItemByCategory("classified");
////////////////////////////////////////////////////////////////////////////////////////////////////

$sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'N' WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_classified_category'";
mysql_query($sqlUpdate, $link);

////////////////////////////////////////////////////////////////////////////////////////////////////
$time_end = getmicrotime();
$time = $time_end - $time_start;
print "Count " . string_ucwords(CLASSIFIED_FEATURE_NAME) . " Category Update on Domain " . SELECTED_DOMAIN_ID . " - " . date("Y-m-d H:i:s") . " - " . round($time,
        2) . " seconds.\n";
////////////////////////////////////////////////////////////////////////////////////////////////////
