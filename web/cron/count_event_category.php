...
# ----------------------------------------------------------------------------------------------------
# * FILE: /cron/count_event_category.php
# ----------------------------------------------------------------------------------------------------

##################################################
# THIS SCRIPT IS ONLY NECESSARY TO COUNT EVENTS BY CATEGORY
# WHEN EVENTS ARE INSERTED/UPDATED OUT OF EDIRECTORY PROCCESS.
# IF IT HAPPENED, RUN THIS SCRIPT ONLY ONE TIME. THE CODE ALREADY
# HANDLES THE NUMBER OF ACTIVE EVENTS BY CATEGORY. TO RUN THIS
...
$sqlDomain = "	SELECT
					...
					WHERE CC.`running` = 'N'
					AND CC.`type` = 'count_event_category'
					AND D.`status` = 'A'
					$sql_period
					ORDER BY
...
if (mysql_num_rows($resDomain) > 0) {
    ...
    $sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'Y', `last_run_date` = NOW() WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_event_category'";
...
$sqlUpdate = "UPDATE `Control_Cron` SET `running` = 'N' WHERE `domain_id` = " . SELECTED_DOMAIN_ID . " AND `type` = 'count_event_category'";
mysql_query($sqlUpdate, $link);

////////////////////////////////////////////////////////////////////////////////////////////////////
$time_end = getmicrotime();
$time = $time_end - $time_start;
print "Count " . string_ucwords(EVENT_FEATURE_NAME) . " Category Update on Domain " . SELECTED_DOMAIN_ID . " - " . date("Y-m-d H:i:s") . " - " . round($time,
        2) . " seconds.\n";
////////////////////////////////////////////////////////////////////////////////////////////////////
