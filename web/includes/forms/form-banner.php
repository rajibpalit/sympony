...
?>
    ...
    <div class="col-md-7">
        ...
        <div class="panel panel-form">
            ...
            <div class="panel-body">
                ...
                <?
                system_fieldsGuide($arrayTutorial, $counterTutorial, system_showText(LANG_LABEL_IMPRESSIONS), "tour-impressions");
                ?>
                <div class="form-group form-inline">
                ...
                <? } elseif ((sess_isAccountLogged()) && (string_strpos($url_base, "/".MEMBERS_ALIAS.""))) { ?>
                    ...
                    <div class="form-group form-inline">
                        <div class="radio" id="tour-expiration">
                            <label>
                                <input <? if ($process == "signup") { echo "disabled=disabled"; } ?> type="radio" name="expiration_setting" id="expiration_setting" value="<?=BANNER_EXPIRATION_RENEWAL_DATE?>" <?=((!$expiration_setting || $expiration_setting == BANNER_EXPIRATION_RENEWAL_DATE) ? "checked" : "" )?> onclick="bannerDisableImpressions()">
                    ...
                    <div class="form-group form-inline">
                        <div class="radio" id="tour-expiration">
                            <label>
                                <input <? if ($process == "signup") { echo "disabled=disabled"; } ?> type="radio" name="expiration_setting" id="expiration_setting" value="<?=BANNER_EXPIRATION_IMPRESSION?>" <?=(($expiration_setting == BANNER_EXPIRATION_IMPRESSION) ? "checked" : "" )?> onclick="bannerDisableRenewalDate()">
        ...
        <div class="panel panel-form">
            ...
            <div class="panel-body">
                <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="radio-inline"><label><input type="radio" id="section" name="section" value="general" checked="checked" onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_GENERALPAGES);?></label></div>
                            <div class="radio-inline"><label><input type="radio" id="section" name="section" value="listing" <? if ($section == "listing") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_LISTING_FEATURE_NAME);?></label></div>
                            <? if (PROMOTION_FEATURE == "on" && CUSTOM_PROMOTION_FEATURE == "on" && CUSTOM_HAS_PROMOTION == "on") { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="promotion" <? if ($section == "promotion") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_PROMOTION_FEATURE_NAME);?></label></div>
                            <? } ?>
                            <? if (EVENT_FEATURE == "on" && CUSTOM_EVENT_FEATURE == "on") { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="event" <? if ($section == "event") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_EVENT_FEATURE_NAME);?></label></div>
                            <? } ?>
                            <? if (CLASSIFIED_FEATURE == "on" && CUSTOM_CLASSIFIED_FEATURE == "on") { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="classified" <? if ($section == "classified") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_CLASSIFIED_FEATURE_NAME);?></label></div>
                            <? } ?>
                            <? if (ARTICLE_FEATURE == "on" && CUSTOM_ARTICLE_FEATURE == "on") { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="article" <? if ($section == "article") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_ARTICLE_FEATURE_NAME);?></label></div>
                            <? } ?>
                            <? if (BLOG_FEATURE == "on" && CUSTOM_BLOG_FEATURE == "on") { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="blog" <? if ($section == "blog") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_BLOG_FEATURE_NAME);?></label></div>
                            <? } ?>
                            <? if (string_strpos($url_base, "/".SITEMGR_ALIAS."") ) { ?>
                                <div class="radio-inline"><label><input type="radio" id="section" name="section" value="global" <? if ($section == "global") echo "checked=\"checked\""; ?> onclick="fillBannerCategorySelect('<?=DEFAULT_URL?>', this.form.category_id, this.value, this.form, <?=SELECTED_DOMAIN_ID?>, 'banner');"> <?=system_showText(LANG_SITEMGR_BANNER_GLOBAL);?></label></div>
                            <? } ?>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="radio-inline">
                                <label>
                                    <input type="radio" name="target_window" value="1" <? if ($target_window == "1") echo "checked";?>>
                                    <?=system_showText(LANG_NO);?>
                                </label>
                            </div>
                            <div class="radio-inline">
                                <label>
                                    <input type="radio" name="target_window" value="2" <? if (($target_window == "2") || (!$target_window)) echo "checked";?>>
                ...
                <div class="form-group">
                    <label for="inputurl2"><?=system_showText(LANG_LABEL_DISPLAY_URL)?> <small class="text-muted">(<?=system_showText(LANG_MSG_MAX_30_CHARS)?>)</small></label>
    ...
    <div class="col-md-5">
        ...
        <div class="panel panel-form-media" id="banner_with_images">
            <div class="panel-heading">
                <?=system_showText(LANG_LABEL_FILE)."<small> (".$levelObj->getWidth($type)."px x ".$levelObj->getHeight($type)."px)</small>";?>
            </div>
            ...
            <div class="panel-footer text-center">
                ...
                <p class="small text-muted"><?=system_showText(LANG_MSG_ALLOWED_FILE_TYPES)?>: SWF, GIF, JPEG, PNG</p>
