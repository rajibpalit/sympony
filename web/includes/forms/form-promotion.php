    ...
    */
    ...
    JavaScriptHandler::registerLoose('
        function setListingSelectBox(){
            $.post( "'.DEFAULT_URL.'/includes/code/deal_ajax_interface.php", { listingId : '.($aux_listing_id ? $aux_listing_id : 0).', action : "getAllListings", accountId : window.selectAcc.val() }).done(function( data ) {
                ...
                {
                    $("#listingSelectBox").html( "'.system_showText( LANG_ATTACHLISTING_UNAVAILABLE ).'" );
                }
            });
        }
    ');
...
?>
    ...
    <div class="col-md-7">
        ...
        <div class="form-group" id="tour-title">
            ...
            <input type="text" class="form-control input-lg" id="name" name="name" value="<?=$name?>" maxlength="100" <?=(!$id) ? " onblur=\"easyFriendlyUrl(this.value, 'friendly_url', '".FRIENDLYURL_VALIDCHARS."', '".FRIENDLYURL_SEPARATOR."');\" " : ""?> placeholder="<?=system_showText(LANG_HOLDER_PROMOTIONTITLE)?>" required>
        ...
        <div class="panel panel-form">
            ...
            <div class="panel-body">
                ...
                <div class="form-group" id="tour-summary">
                    ...
                    <textarea id="summary" name="description" class="textarea-counter form-control <?=($highlight == "description" && !$description ? "highlight" : "")?>" rows="3" data-chars="250" data-msg="<?=system_showText(LANG_MSG_CHARS_LEFT)?>" placeholder="<?=system_showText(LANG_HOLDER_PROMOTIONSUMMARY);?>"><?=$description;?></textarea>
                </div>
        ...
        <div class="panel panel-form" id="tour-discount">
            ...
            <div class="panel-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-6">
                        <label for="deal_price_int"><?=system_showText(LANG_LABEL_DISC_AMOUNT)?> </label>
                        <div class="input-group">
    ...
    <div class="col-md-5">
        ...
        <div class="panel panel-form-media">
            <div class="panel-heading">
                ...
                <span class="btn btn-sm btn-danger delete pull-right <?=(!$cover_id ? "hidden" : "")?>" id="buttonReset">
                    <i class="icon-ion-ios7-trash-outline" onclick="sendCoverImage('promotion', '<?=$_SERVER["PHP_SELF"]?>', <?=($account_id ? $account_id : 0 )?>, 'deleteCover');" ></i>
                </span>
                <div class="pull-right">
                    <input type="file" name="cover-image" class="file-noinput" onchange="sendCoverImage('promotion', '<?=$_SERVER["PHP_SELF"]?>', <?=($account_id ? $account_id : 0 )?>, 'uploadCover');">
                </div>
            </div>
            <div class="panel-body">
        ...
        <?php
            ...
            $imageUploader->buildform( "tour-image", false );
        ?>
    </div>