...
?>
    ...
    <?   
        if (is_object($status) && string_strpos($_SERVER["PHP_SELF"], "/account/") === false) {
            if ($manageModule == "blog") {
                $statusNames = Array(system_showText(LANG_LABEL_ACTIVE), system_showText(LANG_LABEL_SUSPENDED), system_showText(LANG_LABEL_PENDING));
                $statusValues = Array("A", "S", "P");
            } else {
        ...
        <div class="btn-group btn-group-xs">
            ...
            <div class="btn-group btn-group-xs dropup group-ordering " id="paginationSorting">
                ...
                <ul class="dropdown-menu" role="menu" data-group="#paginationSorting">
                    <li class="option-dropup" onclick="window.location = '<?=$paging_url."?".$url_search_params?>'"><span><?=system_showText(LANG_PAGING_ORDERBYPAGE_LASTUPDATE);?></span></li>
                    <li class="option-dropup" onclick="window.location = '<?=$paging_url."?".$url_search_params."&amp;order_by=".($manageModule == "promotion" ? "name" : ($manageModule == "banner" ? "caption" : "title"))."_asc"?>'"><span><?=system_showText(LANG_LABEL_TITLE);?></span></li>
                    <? if ($manageModule != "promotion") { ?>
                    <? if ($manageModule != "article" && $manageModule != "blog") { ?>
                    <li class="option-dropup" onclick="window.location = '<?=$paging_url."?".$url_search_params."&amp;order_by=level_desc"?>'"><span><?=system_showText(LANG_LABEL_LEVEL);?></span></li>
                    <? } ?>
                    <li class="option-dropup" onclick="window.location = '<?=$paging_url."?".$url_search_params."&amp;order_by=status_asc"?>'"><span><?=system_showText(LANG_LABEL_STATUS);?></span></li>
                    <? } ?>
                </ul>
            </div>
