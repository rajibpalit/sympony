<?
    ...
    */
    ...
    if (string_strpos($_SERVER["PHP_SELF"], "/sites/site.php") !== false) {
        ...
        $close = true;
    } elseif (string_strpos($_SERVER["PHP_SELF"], "/wordpress") !== false || string_strpos($_SERVER["PHP_SELF"], "/sugarcrm") !== false) {
        $modalMessage = system_showText(LANG_SITEMGR_PLUGIN_DEMO_MESSAGE);
        $close = true;
    } elseif (string_strpos($_SERVER["PHP_SELF"], "/language") !== false) {
        $modalMessage = system_showText(LANG_SITEMGR_LANGUAGE_DEMO_MESSAGE);
        $close = true;
    } elseif (string_strpos($_SERVER["PHP_SELF"], "/general-settings/index.php") !== false) {
        $modalMessage = system_showText(LANG_SITEMGR_PLUGIN_DEMO_MESSAGE);
        $close = true;
    } elseif (string_strpos($_SERVER["PHP_SELF"], "/layout-editor") !== false) {
        $modalMessage = system_showText(LANG_SITEMGR_THEME_DEMO_MESSAGE);
        $close = true;
    } elseif (
            string_strpos($_SERVER["PHP_SELF"], "/content.php") !== false || 
            string_strpos($_SERVER["PHP_SELF"], "basic-information/index.php") !== false || 
            string_strpos($_SERVER["PHP_SELF"], "content/slider/index.php") !== false || 
            string_strpos($_SERVER["PHP_SELF"], "contentlevel.php") !== false || 
            string_strpos($_SERVER["PHP_SELF"], "pages/custom.php") !== false|| 
            string_strpos($_SERVER["PHP_SELF"], "site-navigation/index.php") !== false || 
            string_strpos($_SERVER["PHP_SELF"], "lead-editor/index.php") !== false ||
