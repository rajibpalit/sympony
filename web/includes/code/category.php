<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if (validate_form("category", $_POST, $message_category)) {
            ...
            if ($_FILES['image']['name'] && $_FILES['image']['tmp_name'] && !$_FILES['image']['error']) {
                $imageArray = image_uploadForItem($_FILES['image']['tmp_name'], "sitemgr_", IMAGE_CATEGORY_FULL_WIDTH, IMAGE_CATEGORY_FULL_HEIGHT, IMAGE_CATEGORY_THUMB_WIDTH, IMAGE_CATEGORY_THUMB_HEIGHT);
	...
	if ($id) {
		...
		$category = db_getFromDB(string_strtolower($table_category), "id", db_formatNumber($id), 1, "", "object", SELECTED_DOMAIN_ID);
		$category->extract();
