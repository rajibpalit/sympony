...
?>
    <div class="row" id="div_location_<?=$each_location_level?>" <?= (${"locations".$each_location_level} & $_POST["new_location".$each_location_level."_field"]=="") ? "" : "style=\"display:none;\""?>>
        <div class="col-sm-4">
        ...
        <div class="col-sm-4 field" id="div_select_<?=$each_location_level?>">
            <select class="form-control" name="location_<?=$each_location_level?>" id="location_<?=$each_location_level?>" <? if (($each_location_child_level) and ($_location_father_level != $each_location_level)) { ?> onchange="loadLocationSitemgrMembers('<?=DEFAULT_URL?>', '<?=EDIR_LOCATIONS?>', <?=$each_location_level?>, <?=$each_location_child_level?>, this.value);" <? } ?>>
...
<?php           if ($each_location_level != 1 and !string_strpos($_SERVER["PHP_SELF"], "search.php") and ($_location_level != $each_location_level)) { ?>
    <div class="row"id="div_new_location<?=$each_location_level?>_field" <?=($_POST["new_location".$each_location_level."_field"]!=""?"":($_POST["new_location".$each_location_father_level."_field"]!=""?"":"style=\"display:none;\""))?> >
        <div class="col-sm-4">
