<?php
/**
 * $Id: FileFilter.php 834 2011-08-23 09:17:01Z joakim $
 *
...
class Moxiecode_BasicFileFilter extends Moxiecode_FileFilter {
	...
	function accept(&$file) {
		...
		if (is_array($this->_excludeFolders)) {
			foreach ($this->_excludeFolders as $folder) {
				if (strpos($absPath, $folder) != "") {
					if ($this->_debug)
						mcdebug("File denied \"" . $absPath . "\" by \"excludeFolders\".");
		...
		if (is_array($this->_includeFolders)) {
			...
			if (!$state) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"includeFolders\".");
		...
		if (is_array($this->_excludeFiles) && $isFile) {
			foreach ($this->_excludeFiles as $fileName) {
				if ($name == $fileName) {
					if ($this->_debug)
						mcdebug("File \"" . $absPath . "\" denied by \"excludeFiles\".");
		...
		if (is_array($this->_includeFiles) && $isFile) {
			...
			if (!$state) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"includeFiles\".");
		...
		if ($isFile) {
			if ($this->_dirsOnly) {
				if ($this->_debug)
					mcdebug("File denied \"" . $absPath . "\" by \"dirsOnly\".");
			...
			if ($this->_excludeFilePattern && preg_match($this->_excludeFilePattern, $name)) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"excludeFilePattern\".");
			...
			if ($this->_includeFilePattern && !preg_match($this->_includeFilePattern, $name)) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"includeFilePattern\".");
			...
			if ($this->_filesOnly) {
				if ($this->_debug)
					mcdebug("Dir denied \"" . $absPath . "\" by \"filesOnly\".");
			...
			if ($this->_excludeDirectoryPattern && preg_match($this->_excludeDirectoryPattern, $name)) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"excludeDirectoryPattern\".");
			...
			if ($this->_includeDirectoryPattern && !preg_match($this->_includeDirectoryPattern, $name)) {
				if ($this->_debug)
					mcdebug("File \"" . $absPath . "\" denied by \"includeDirectoryPattern\".");
		...
		if ($this->_includeWildcardPattern && !$this->_fnmatch($this->_includeWildcardPattern, $name)) {
			if ($this->_debug)
				mcdebug("File \"" . $absPath . "\" denied by \"includeWildcardPattern\".");
		...
		if ($this->_excludeWildcardPattern && $this->_fnmatch($this->_excludeWildcardPattern, $name)) {
			if ($this->_debug)
				mcdebug("File \"" . $absPath . "\" denied by \"excludeWildcardPattern\".");
	...
	function _fnmatch($pattern, $file) {
		...
		return preg_match("/$pattern/", strtolower($file));
	}
