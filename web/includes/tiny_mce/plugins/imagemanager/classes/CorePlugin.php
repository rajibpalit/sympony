<?php
/**
 * $Id: CorePlugin.php 833 2011-08-23 09:10:23Z joakim $
 *
			...
			if ($man->verifyPath($path)) {
				...
				if (isset($input["chunk"])) {
					...
					if ($_COOKIE["SECTION_SITEMGR"]) {
						$filename = "sitemgr_".$input["name" . $i];
					}else if ($_COOKIE["SECTION_MEMBERS"]) {
						$filename = $_SESSION[SESS_ACCOUNT_ID].'_'.$input["name" . $i];
					}else{
						$filename = $input["name"];
					}
					...
					if (!checkBool($config['general.demo'])) {
						...
						if ($chunk == $chunks - 1) {
							clearstatcache();
							//mcdebug($chunk, $chunks, filesize($file->getAbsolutePath()), $chunk == 0 ? 'wb' : 'ab');
							$file->importFile();
						}
					}
...
//						$filename = utf8_encode($input["name" . $i]);
                        /* eDirectory fix for basic uploader */
                        if ($_COOKIE["SECTION_SITEMGR"]) {
                            $filename = "sitemgr_".$input["name" . $i];
                        }else if ($_COOKIE["SECTION_MEMBERS"]) {
                            $filename = $_SESSION[SESS_ACCOUNT_ID].'_'.$input["name" . $i];
