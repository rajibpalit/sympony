<?php
    ...
    if (($_SERVER['REQUEST_METHOD'] == "POST")) {
        ...
        if ($validate_account && $validate_contact && $validate_classified && $validate_discount) {
            ...
            $sitemgr_msg .= "<br /><a href=\"".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/content/".CLASSIFIED_FEATURE_FOLDER."/classified.php?id=".$classified->getNumber("id")."\" target=\"_blank\">".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/content/".CLASSIFIED_FEATURE_FOLDER."/classified.php?id=".$classified->getNumber("id")."</a><br /><br />";
    ...
    $formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".MEMBERS_ALIAS."/login.php?destiny=".EDIRECTORY_FOLDER."/".MEMBERS_ALIAS."/".CLASSIFIED_FEATURE_FOLDER."/classified.php";
    ...
    if ($classifiedLevelObj->getPrice($level) > 0) {
        $labelPrice = CURRENCY_SYMBOL.$classifiedLevelObj->getPrice($level);
        $labelPriceRenewal = " ".system_showText(LANG_PER)." ";
        if (payment_getRenewalCycle("classified") > 1) {
            $labelPriceRenewal .= payment_getRenewalCycle("classified")." ";
            $labelPriceRenewal .= payment_getRenewalUnitNamePlural("classified");
        } else {
            $labelPriceRenewal .= payment_getRenewalUnitName("classified");
        }
    } else {
    ...
    # SITE CONTENT
    # ----------------------------------------------------------------------------------------------------
    $array_HeaderContent = front_getSiteContent("Advertise with Us");
