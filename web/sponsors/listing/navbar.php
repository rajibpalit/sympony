<?php
    ...
    if (
        (BACKLINK_FEATURE == "on" && $listingHasBacklink == "y") ||
        (TWILIO_APP_ENABLED == "on" && TWILIO_APP_ENABLED_CALL == "on"  && $listingHasClickToCall == "y")
        ) {
?>
    ...
    <nav>
        <ul class="nav nav-tabs nav-justified">
            ...
            <? if (BACKLINK_FEATURE == "on" && $listingHasBacklink == "y") { ?>
                <li <?=((string_strpos($_SERVER["PHP_SELF"], "/".MEMBERS_ALIAS."/".LISTING_FEATURE_FOLDER."/backlinks") !== false) ? "class=\"active\"" : "") ?>>
                    <a href="<?=DEFAULT_URL?>/<?=MEMBERS_ALIAS?>/<?=LISTING_FEATURE_FOLDER?>/backlinks.php?id=<?=$id?>"><?=system_showText(LANG_LABEL_BACKLINK)?></a>
                </li>
            <? } ?>
