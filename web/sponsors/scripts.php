        ...
        if (url){
            ...
            $.post(DEFAULT_URL + "/check_website.php", {
                ...
                if ($.trim(response) == "OK"){
                    ...
                    $('#backlinkValid').val("1");
                    alert('<?=system_showText(LANG_LABEL_VALIDATION_OK)?>');
                } else {
                    $('#continue').prop("disabled", "disabled");
                    $('#continue').addClass("inactive");
                    $('#continue').css("cursor" , "default");
                    $('#backlinkValid').val("0");
                    alert('<?=system_showText(LANG_LABEL_VALIDATION_FAIL)?>');
                }
            });
        } else {
            alert('<?=system_showText(LANG_LABEL_TYPE_URL)?>');
        }
    }
