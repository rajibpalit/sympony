<?php
    ...
    if (($_SERVER['REQUEST_METHOD'] == "POST")) {
        ...
        if ($validate_account && $validate_contact && $validate_event && $validate_discount) {
            ...
            $sitemgr_msg .= "<br /><a href=\"".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/content/".EVENT_FEATURE_FOLDER."/event.php?id=".$event->getNumber("id")."\" target=\"_blank\">".((SSL_ENABLED == "on" && FORCE_SITEMGR_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".SITEMGR_ALIAS."/content/".EVENT_FEATURE_FOLDER."/event.php?id=".$event->getNumber("id")."</a><br /><br />";
    ...
    $formloginaction = ((SSL_ENABLED == "on" && FORCE_MEMBERS_SSL == "on") ? SECURE_URL : NON_SECURE_URL)."/".MEMBERS_ALIAS."/login.php?destiny=".EDIRECTORY_FOLDER."/".MEMBERS_ALIAS."/".EVENT_FEATURE_FOLDER."/event.php";
    ...
    if ($eventLevelObj->getPrice($level) > 0) {
        $labelPrice = CURRENCY_SYMBOL.$eventLevelObj->getPrice($level);
        $labelPriceRenewal = " ".system_showText(LANG_PER)." ";
        if (payment_getRenewalCycle("event") > 1) {
            $labelPriceRenewal .= payment_getRenewalCycle("event")." ";
            $labelPriceRenewal .= payment_getRenewalUnitNamePlural("event");
        } else {
            $labelPriceRenewal .= payment_getRenewalUnitName("event");
        }
    } else {
