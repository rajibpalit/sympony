...
class LocationFilter extends BaseFilter
{
    ...
    public function getEnabledLocations()
    {
        ...
        or
        $this->enabledLocations = $this->container->get('doctrine')->getRepository('WebBundle:SettingLocation')
            ->getLocationsEnabledID() ?: [1, 3, 4];
    ...
    public function registerItem(SearchEvent $searchEvent, $eventName)
    {
        ...
        if ($locationFriendlyURLs = $parameterInfo->getRouteParameter("location")) {

            $searchedLocations = $searchEngine->locationFriendlyURLSearch($locationFriendlyURLs);
            ...
            foreach ($searchedLocations as $id => $location) {
                ...
                $locationArray[$location->getLevel()] = preg_replace("/L\d+\:/", "", $id);

                $this->container->get("reporthandler")->addLocationSearchReport(
    ...
    public function registerMapItem(SearchEvent $searchEvent, $eventName)
    {
        ...
        /* will only add filters if there are location information inside the request */
        if ($locationFriendlyURLs = $this->container->get("search.parameters")->getRouteParameter("location")) {
            $searchEngine = $this->container->get("search.engine");
            $searchedLocations = $searchEngine->locationFriendlyURLSearch($locationFriendlyURLs);
            ...
            if ($results = $this->getRecursiveLocations($searchedLocations)) {
                ...
                $this->addElasticaFilter(
                    SearchEngine::getElasticaQueryBuilder()->filter()->terms("locationId", $locationIds)
                );
            }
        }
    }
    ...
    public function getFilterView()
    {
        ...
        if ($this->aggregationInfo) {
            ...
            /* Gets the friendlyUrl of the locations being searched for */
            $requestFilterLocations = $this->container->get("search.parameters")->getRouteParameter("location");
            ...
            $locationDictionary = [];
            $this->extractFromResultSet($resultLocations, $locationDictionary, $requestFilterLocations);
...
//            }
            ...
            $return = $this->container->get("twig")->render(
                ...
                ["locationTree" => $locationTree, "selected" => $requestFilterLocations]
            );
        }
     ...
     * @param array $marked
     */
    public function extractFromResultSet($locations, &$dictionary, $marked = [])
    {
        $parents = [];
        ...
        foreach ($locations as $location) {
            ...
            $dictionary[$location->getId()] = new FilterMenuTreeNode(
                ...
                $location->getLevel(),
                $location->getId(),
                in_array($location->getFriendlyUrl(), $marked),
                null,
                $locationCount
            );
        }
        ...
        if ($parents) {
            ...
            $this->extractFromResultSet($resultLocations, $dictionary, $marked);
        }
    }
     ...
     */
    private function linkElements(&$locationDictionary)
    {
        $selectedItemParents = [];
        $parentlessItems = [];
    ...
    private function getSearchPageUrl($location)
    {
        ...
        if ($location->friendlyUrl) {
            ...
            if ($location->module == $this->getLastLevel()) {
                /* Clears everything that is not a direct sibling */
                if ($parent = $location->parent) {
                    ...
                    $selectedSiblings = array_intersect($siblings, $searchParameters->getRouteParameter("location"));

                    $searchParameters->clearRouteParameter("location");

                    foreach ($selectedSiblings as $sibling) {
                        $searchParameters->addRouteParameter("location", $sibling);
                    }
                }

                $searchParameters->toggleRouteParameter("location", $location->friendlyUrl);
            } else {
                if (in_array($location->friendlyUrl, $searchParameters->getRouteParameter("location"))) {
                    $searchParameters->clearRouteParameter("location");
                } else {
                    $searchParameters->clearRouteParameter("location");
                    $searchParameters->addRouteParameter("location", $location->friendlyUrl);
                }
            }
     ...
     */
    private function setSearchUrls($locationTree)
    {
     ...
     * @return int
     */
    private function bubbleCounts($location)
    {
     ...
     * @return int
     */
    private function locationSort($a, $b)
    {
        $result = 0;
