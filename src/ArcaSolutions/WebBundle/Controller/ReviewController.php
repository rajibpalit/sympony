...
class ReviewController extends Controller
{
    ...
    public function addReviewAction(Request $request)
    {
        ...
        if ($info = json_decode($this->get("url_encryption")->decrypt($request->request->get("info")))
            ...
            $setKey = $module == "listing" ? 'listing_rate' : 'article_rate';
            $forceLogin = $settings->getSettingSocialNetwork($setKey) === 'yes' ? true : false;
            ...
            if (isset($review) and $review) {
                ...
                $form = $this->createForm(new ReviewsType($memberAccount));
                $form->handleRequest($request);

                if ($form->isValid()) {
                    try {
                        $review = $this->getDoctrine()->getRepository('WebBundle:Review')
                            ->addReview($module, $id, $memberAccount, $form);
                        ...
                        $settingRepository = $this->getDoctrine()->getRepository('WebBundle:Setting');
                        $reviewEnabled = $settingRepository->getSetting('review_approve');

                        $response = [
                            "status"  => true,
                            "content" => $reviewEnabled ?
                                $this->get('translator')->trans('Your review has been submitted for approval.') :
                                $this->get('translator')->trans('Thank you for submitting your review!'),
                        ];
                    } catch (\Exception $e) {
