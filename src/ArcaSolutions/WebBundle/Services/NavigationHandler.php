...
final class NavigationHandler
{
    ...
    {
        $menu = $this->doctrine->getRepository('WebBundle:SettingNavigation')
            ->getMenuByThemeAndArea($this->multidomainSettings->getTemplate(), 'header');
    ...
    private function removesDisabledModules(&$menu = [])
    {
        ...
        foreach ($menu as $key => $item) {
            ...
            if (false === strpos($item->getLink(), 'DEFAULT_URL')) {
                continue;
            }
    ...
    private function translateLinks($menu = [])
    {
        ...
        foreach ($menu as $item) {
            ...
            if (false !== strpos($link, 'SECURE_URL')) {
                /* homepage */
                $link = 'web_homepage';
            ...
            /* it should changes the link of advertise, terms, privacy */
    ...
    private function replacesRoutes(SettingNavigation $item)
    {
        ...
        /* advertise|terms|privacy must be a custom link, for now */
        if (strpos($item->getLink(), $this->container->getParameter('alias_advertise_url_divisor')) !== false
            || strpos($item->getLink(), $this->container->getParameter('alias_terms_url_divisor')) !== false
    ...
    private function getRoutesToReplace()
    {
        return [
            ...
            'web_lead' => 'web_enquire',
            'web_advertise' => '/' . $this->container->getParameter('alias_advertise_url_divisor'),
    ...
    {
        $menu = $this->doctrine->getRepository('WebBundle:SettingNavigation')
            ->getMenuByThemeAndArea($this->multidomainSettings->getTemplate(), 'footer');
